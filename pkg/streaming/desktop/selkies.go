...
func (p *SelkiesProvider) Start(ctx context.Context, opts streaming.StreamOptions) (*streaming.StreamInfo, error) {
	...
	return &streaming.StreamInfo{
		...
		InputEnabled: config.EnableInput,
...
func (p *SelkiesProvider) Stop(ctx context.Context, streamID string) error {
	p.mu.Lock()
	...
	p.logger.Info("stopped stream", zap.String("stream_id", streamID))

	return nil
}

// GetStatus returns the current status of a stream.
func (p *SelkiesProvider) GetStatus(ctx context.Context, streamID string) (*streaming.StreamInfo, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	...
	case ProcessStateError:
		state = streaming.StreamStateError
	case ProcessStateStarting:
		state = streaming.StreamStateStarting
	case ProcessStateStopping:
		state = streaming.StreamStateStopping
	}
	...
	return &streaming.StreamInfo{
		...
		ICEServers:   stream.config.ICEServers,
		Resolution:   stream.config.Resolution,
		FrameRate:    stream.config.FrameRate,
		BitRate:      stream.config.BitRate,
		AudioEnabled: stream.config.AudioEnabled,
		InputEnabled: stream.config.EnableInput,
		StartedAt:    stream.startedAt,
		Error:        stream.error,
	}, nil
}

// UpdateOptions updates stream options.
func (p *SelkiesProvider) UpdateOptions(ctx context.Context, streamID string, opts streaming.StreamOptions) error {
	// Selkies doesn't support runtime option updates easily.
	// Would need to restart the process with new options.
	return &ProviderError{
		Provider:  "selkies",
		Operation: "update_options",
		Message:   "runtime option updates not supported",
	}
}

// HealthCheck verifies the provider is functioning.
func (p *SelkiesProvider) HealthCheck(ctx context.Context) error {
	// Check if selkies-gstreamer is available
	path, err := exec.LookPath(p.config.ExecutablePath)
	if err != nil {
		return &ProviderError{
			Provider:  "selkies",
	...
	p.logger.Debug("health check passed", zap.String("executable", path))
	return nil
}
...
func (p *SelkiesProvider) GetDisplayInfo(ctx context.Context) ([]DisplayInfo, error) {
	...
	_ = output // TODO: Parse output for actual dimensions
...
func (p *SelkiesProvider) InputHandler() streaming.InputHandler {
...
func (p *SelkiesProvider) mergeConfig(opts streaming.StreamOptions) SelkiesConfig {
	...
	config.AudioEnabled = opts.EnableAudio
	config.EnableInput = opts.EnableInput
