...
// to the publisher for processing.
...
// SetPublisherChannel sets the data channel connected to the publisher.
...
func (ic *InputChannel) AddSubscriberChannel(peerID string, dc *webrtc.DataChannel) error {
	ic.mu.Lock()
	defer ic.mu.Unlock()

	if ic.closed {
		return streaming.ErrInputChannelClosed
	}
...
// IsClosed returns whether the input channel is closed.
func (ic *InputChannel) IsClosed() bool {
	ic.mu.RLock()
	defer ic.mu.RUnlock()
	return ic.closed
}

// HasPublisherChannel returns whether a publisher channel is set.
func (ic *InputChannel) HasPublisherChannel() bool {
	ic.mu.RLock()
	defer ic.mu.RUnlock()
	return ic.publisherChannel != nil
}

// SubscriberCount returns the number of subscriber channels.
func (ic *InputChannel) SubscriberCount() int {
	ic.mu.RLock()
	defer ic.mu.RUnlock()
	return len(ic.subscriberChannels)
}

// InputChannelStats contains input channel statistics.
...
// Stats returns current input channel statistics.
func (ic *InputChannel) Stats() InputChannelStats {
	ic.mu.RLock()
	defer ic.mu.RUnlock()
