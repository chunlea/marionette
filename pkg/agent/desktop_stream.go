...
// DesktopStreamState tracks state of an active stream.
...
type DesktopStreamManager struct {
	...
	mu      sync.RWMutex
	streams map[string]*DesktopStreamState // streamID -> DesktopStreamState
}
...
func NewDesktopStreamManager(config DesktopStreamConfig, logger *zap.Logger) *DesktopStreamManager {
	...
	return &DesktopStreamManager{
		...
		streams:  make(map[string]*DesktopStreamState),
	}
}
...
func (m *DesktopStreamManager) HandleStartDesktopStream(ctx context.Context, cmd *pb.StartDesktopStream) (*pb.RunnerMessage, error) {
	...
	m.mu.Lock()
	m.streams[cmd.StreamId] = &DesktopStreamState{
...
func (m *DesktopStreamManager) HandleStopDesktopStream(ctx context.Context, cmd *pb.StopDesktopStream) (*pb.RunnerMessage, error) {
	...
	if !exists {
		...
		return m.buildStoppedResponse(cmd.StreamId, "", "stream not found"), nil
	}
	...
	// Update state
	m.mu.Lock()
	if s, ok := m.streams[cmd.StreamId]; ok {
		now := time.Now()
		s.Status = "stopped"
		s.StoppedAt = &now
	}
	...
	return m.buildStoppedResponse(cmd.StreamId, state.SessionID, cmd.Reason), nil
}
...
func (m *DesktopStreamManager) GetStreamState(streamID string) (*DesktopStreamState, bool) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *DesktopStreamManager) buildStreamOptions(cmd *pb.StartDesktopStream) streaming.StreamOptions {
	...
	if cfg := cmd.Config; cfg != nil {
		...
		if cfg.Display != "" {
			opts.Metadata = map[string]string{"display": cfg.Display}
		} else if m.config.DefaultDisplay != "" {
			opts.Metadata = map[string]string{"display": m.config.DefaultDisplay}
		}
		opts.AudioEnabled = cfg.AudioEnabled
		opts.InputEnabled = cfg.InputEnabled
	}
...
func (m *DesktopStreamManager) buildStartedResponse(cmd *pb.StartDesktopStream, info *streaming.StreamInfo) *pb.RunnerMessage {
	var width, height, frameRate int32
